...
import CInputForm from "./adjustment-reason-input-form/adjustment-reason-input-form.vue";
...
export default class ShiftConfigurations extends Vue {
  ...
  handleDelete(params: any) {
    ...
    this.dialogMessage = this.$t(
      "messages.employee.confirm.deleteAdjustmentReason"
    );
  ...
  async loadDataGrid(search: any = this.searchDefault) {
    try {
      ...
      const { data } = await salaryAdjustmentAPI.GetAdjustmentReasonList(
        params
      );
      if (data) {
        this.rowData = data;
  ...
  async loadEditData(id: any) {
    try {
      const { data } = await salaryAdjustmentAPI.GetAdjustmentReason(id);
      if (data) {
        this.$nextTick(() => {
  ...
  async insertData(formData: any) {
    try {
      const { status2 } = await salaryAdjustmentAPI.InsertAdjustmentReason(
        formData
      );
      if (status2.status == 0) {
        getToastSuccess(
          this.$t("messages.employee.success.saveAdjustmentReason")
        );
        this.loadDataGrid(this.searchDefault);
        this.showForm = false;
      }
  ...
  async updateData(formData: any) {
    try {
      const { status2 } = await salaryAdjustmentAPI.UpdateAdjustmentReason(
        formData
      );
      if (status2.status == 0) {
        getToastSuccess(
          this.$t("messages.employee.success.updateAdjustmentReason")
        );
        this.loadDataGrid(this.searchDefault);
        this.showForm = false;
      }
  ...
  async deleteData() {
    try {
      const { status2 } = await salaryAdjustmentAPI.DeleteAdjustmentReason(
        this.deleteParam
      );
      if (status2.status == 0) {
        getToastSuccess(
          this.$t("messages.employee.success.deleteAdjustmentReason")
        );
        this.loadDataGrid(this.searchDefault);
