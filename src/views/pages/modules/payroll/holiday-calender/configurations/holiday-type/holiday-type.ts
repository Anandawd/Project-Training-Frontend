...
import CInputForm from "./holiday-type-input-form/holiday-type-input-form.vue";
...
export default class ShiftConfigurations extends Vue {
  ...
  async loadDataGrid(search: any = this.searchDefault) {
    try {
      ...
      const { data } = await holidayCalenderAPI.GetHolidayTypeList(params);
      if (data) {
        this.rowData = data;
      } else {
        this.rowData = [];
  ...
  async loadEditData(id: any) {
    try {
      const { data } = await holidayCalenderAPI.GetHolidayType(id);
      if (data) {
        this.$nextTick(() => {
  ...
  async insertData(formData: any) {
    try {
      const { status2 } = await holidayCalenderAPI.InsertHolidayType(formData);
      if (status2.status == 0) {
        getToastSuccess(this.$t("messages.attendance.success.saveHolidayType"));
        this.loadDataGrid(this.searchDefault);
        this.showForm = false;
      }
    } catch (error) {
  ...
  async updateData(formData: any) {
    try {
      const { status2 } = await holidayCalenderAPI.UpdateHolidayType(formData);
      if (status2.status == 0) {
        getToastSuccess(
          this.$t("messages.attendance.success.updateHolidayType")
        );
        this.loadDataGrid(this.searchDefault);
        this.showForm = false;
      }
  ...
  async deleteData() {
    try {
      const { status2 } = await holidayCalenderAPI.DeleteHolidayType(
        this.deleteParam
      );
      if (status2.status == 0) {
        getToastSuccess(
          this.$t("messages.attendance.success.deleteHolidayType")
        );
        this.loadDataGrid(this.searchDefault);
