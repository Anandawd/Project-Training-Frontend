...
const resourceAPI = new ConfigurationResource("Country");
...
export default class Country extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.cfgInitCountry,
            this.paramsData.id
          ),
      },
    ];
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("Continent");
      this.inputFormElement.listDropdown.continent = data;
    } catch (error) {
      getError(error);
    }
  }
  ...
  beforeMount() {
    ...
    this.searchOptions = [
      ...
      { text: this.$t("commons.filter.continent"), value: 2 },
      { text: this.$t("commons.filter.isoCode"), value: 3 },
    ...
    this.columnDefs = [
      ...
      { headerName: this.$t("commons.table.name"), field: "name", width: 230 },
      {
        headerName: this.$t("commons.table.continent"),
        field: "ContinentName",
        width: 100,
      },
      {
        headerName: this.$t("commons.table.isoCode"),
        field: "iso_code",
        width: 75,
      },
      {
