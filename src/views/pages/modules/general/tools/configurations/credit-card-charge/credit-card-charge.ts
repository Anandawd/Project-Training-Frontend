...
const resourceAPI = new ConfigurationResource("CreditCardCharge");
...
export default class CreditCardCharge extends Vue {
  public rowData: any = [];
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.cfgInitCreditCardCharge,
            this.paramsData.id
          ),
      },
    ];
  ...
  handleDelete(params: any) {
    ...
    this.deleteCode = params.account_code;
  }

  async handleEdit(params: any) {
    this.loadDropdown();
    this.modeData = $global.modeData.edit;
    await this.editData(params.account_code);
  }
  ...
  async handleSave(formData: any) {
    formData.charge_percent = parseFloat(formData.charge_percent);
    this.isSaving = true;
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("AccountEDC");
      this.inputFormElement.listDropdown.accountEdc = data;
    } catch (error) {
      getError(error);
    }
  }
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      {
        headerName: this.$t("commons.table.chargePercent"),
  ...
  get schema() {
    return Yup.object().shape({
      "Account EDC": Yup.string().required(),
      "Charge Percent": Yup.number().test((val) => {
        return val > 0;
      }),
    });
  }
