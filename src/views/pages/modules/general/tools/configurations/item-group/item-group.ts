...
const resourceAPI = new ConfigurationResource("ItemGroup");
...
export default class ItemGroup extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.ItemGroup,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    formData.percentage_cost = parseFloat(formData.percentage_cost);
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("ItemGroupType");
