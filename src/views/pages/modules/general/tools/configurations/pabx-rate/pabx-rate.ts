...
const resourceAPI = new ConfigurationResource("PabxRate");
...
export default class PabxRate extends Vue {
  public rowData: any = [];
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      {
        headerName: this.$t("commons.table.rate"),
        field: "rate",
        width: 60,
      ...
      {
        headerName: this.$t("commons.table.freeSeconds"),
        field: "free_seconds",
        width: 110,
        valueFormatter: formatNumber,
        filterParams: {
  ...
  getContextMenu(params: any) {
    ...
    const result = [
      ...
      "separator",
      {
        ...
        action: () => this.handleShowForm(this.paramsData, 3),
      },
    ];
    return result;
  }
  ...
  async loadDropdown() {
    try {
      const params = ["State", "Regency"];
      const { data } = await resourceAPI.codeNameListArray(params);
      this.inputFormElement.listDropdown.city = data;
    } catch (error) {
      getError(error);
    }
  }
