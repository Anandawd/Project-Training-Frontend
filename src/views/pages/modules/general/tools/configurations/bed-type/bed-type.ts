...
import bedType from "@/services/api/configuration/walk-in-reservation/bed-type/bed-type";
...
export default class BedType extends Vue {
  public rowData: any = [];
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            $global.tableName.cfgInitBedType,
            this.paramsData.id
          ),
      },
    ];
  ...
  async loadData(search: any = this.searchDefault) {
    ...
    try {
      ...
      const { data } = await bedTypeAPI.list(params);
      this.rowData = data;
  ...
  async editData(code: any) {
    try {
      let { data } = await bedTypeAPI.edit(code);
      if (data) {
  ...
  async updateData(formData: any) {
    try {
      ...
      const { status2 } = await bedTypeAPI.update(formData);
      if (status2.status == 0) {
        this.loadData();
        this.showForm = false;
  ...
  async insertData(formData: any) {
    try {
      formData.id_sort = parseInt(formData.id_sort);
      const { status2 } = await bedTypeAPI.create(formData);
      if (status2.status == 0) {
        this.loadData();
        this.showForm = false;
        getToastSuccess(this.$t("messages.saveSuccess"));
  ...
  async deleteData() {
    try {
      const { status2 } = await bedTypeAPI.delete(this.deleteCode);
      if (status2.status == 0) {
        this.loadData();
