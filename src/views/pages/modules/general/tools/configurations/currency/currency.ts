...
const resourceAPI = new ConfigurationResource("Currency");
...
export default class Currency extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            $global.tableName.cfgInitCurrency,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    this.isSaving = true;
    formData.exchange_rate = parseFloat(formData.exchange_rate);
    formData.is_default = parseInt(formData.is_default);
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("Account");
  ...
  beforeMount() {
    ...
    this.searchOptions = [
      ...
      { text: this.$t("commons.filter.accountCode"), value: 2 },
      { text: this.$t("commons.filter.symbol"), value: 3 },
    ...
    this.columnDefs = [
      ...
      { headerName: this.$t("commons.table.name"), field: "name", width: 70 },
      {
      ...
      {
        headerName: this.$t("commons.table.isDefault"),
        field: "is_default",
        width: 70,
  ...
  get schema() {
    return Yup.object().shape({
      ...
      Code: Yup.string()
        ...
        .required()
        .max(3),
      "Account Code": Yup.string().required(),
