...
import roomType from "@/services/api/configuration/walk-in-reservation/room-type/room-type";
...
const roomTypeAPI = new roomType();
...
export default class RoomType extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            $global.tableName.cfgInitRoomType,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    ...
    formData.min_room_left = parseInt(formData.min_room_left);
    formData.cm_percent_availability = parseInt(
      formData.cm_percent_availability
    );
  ...
  async loadData(search: any = this.searchDefault) {
    ...
    try {
      ...
      const { data } = await roomTypeAPI.list(params);
      this.rowData = data;
  ...
  async editData(code: any) {
    try {
      let { data } = await roomTypeAPI.edit(code);
      if (data) {
  ...
  async updateData(formData: any) {
    try {
      ...
      const { status2 } = await roomTypeAPI.update(formData);
      if (status2.status == 0) {
        this.loadData();
        this.showForm = false;
  ...
  async insertData(formData: any) {
    try {
      formData.id_sort = parseInt(formData.id_sort);
      const { status2 } = await roomTypeAPI.create(formData);
      if (status2.status == 0) {
        this.loadData();
        this.showForm = false;
        getToastSuccess(this.$t("messages.saveSuccess"));
  ...
  async deleteData() {
    try {
      const { status2 } = await roomTypeAPI.delete(this.deleteCode);
      if (status2.status == 0) {
        this.loadData();
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      {
        headerName: this.$t("commons.table.cmPercentAvailability"),
        field: "cm_percent_availability",
        width: 160,
        cellStyle: { textAlign: "right" },
        headerClass: "align-header-right",
      },
      {
        headerName: this.$t("commons.table.minRoomLeft"),
        field: "min_room_left",
        width: 110,
        cellStyle: { textAlign: "right" },
        headerClass: "align-header-right",
      ...
      {
        headerName: this.$t("commons.table.isCmFromGlobal"),
        field: "is_cm_from_global",
        width: 115,
  ...
  get schema() {
    return Yup.object().shape({
      ...
      Code: Yup.string()
        ...
        .max(4),
      "ID Sort": Yup.number(),
      "Cm Percent Availability": Yup.number(),
      "Min Room Left": Yup.number(),
    });
  }
