...
const resourceAPI = new ConfigurationResource("RoomRateSubCategory");
...
export default class RoomRateSubCategory extends Vue {
  public rowData: any = [];
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            $global.tableName.cfgInitRoomRateSubCategory,
            this.paramsData.id
          ),
      },
    ];
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("RoomRateCategory");
