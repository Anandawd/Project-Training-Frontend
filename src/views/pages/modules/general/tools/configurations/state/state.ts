...
const resourceAPI = new ConfigurationResource("State");
...
export default class State extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.cfgInitState,
            this.paramsData.id
          ),
      },
    ];
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("Country");
      this.inputFormElement.listDropdown.country = data;
    } catch (error) {
      getError(error);
    }
  }
