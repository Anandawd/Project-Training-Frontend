...
const resourceAPI = new ConfigurationResource("Account");
...
export default class Account extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            $global.tableName.cfgInitAccount,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    this.isSaving = true;
    formData.is_payment = parseInt(formData.is_payment);
    formData.is_refund = parseInt(formData.is_refund);
  ...
  async loadDropdown() {
    try {
      ...
      this.inputFormElement.listDropdown.account = data;
    } catch (error) {
      getError(error);
    }
  }
  ...
  beforeMount() {
    ...
    this.searchOptions = [
      ...
      { text: this.$t("commons.filter.subGroup"), value: 3 },
      { text: this.$t("commons.filter.journalAccount"), value: 4 },
      { text: this.$t("commons.filter.taxAndService"), value: 5 },
      { text: this.$t("commons.filter.itemGroup"), value: 6 },
      { text: this.$t("commons.filter.createdBy"), value: 7 },
    ...
    this.columnDefs = [
      ...
      { headerName: this.$t("commons.table.name"), field: "name", width: 190 },
      {
        headerName: this.$t("commons.table.payment"),
        field: "is_payment",
        width: 80,
      ...
      {
        headerName: this.$t("commons.table.refund"),
        field: "is_refund",
        width: 70,
        cellStyle: { textAlign: "center" },
        headerClass: "align-header-center",
  ...
  get schema() {
    return Yup.object().shape({
      ...
      Code: Yup.string()
        ...
        .required()
        .max(4),
      "Sub Group": Yup.string().required(),
      "Journal Account": Yup.string().required(),
