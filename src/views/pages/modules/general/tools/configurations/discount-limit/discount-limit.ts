...
const resourceAPI = new ConfigurationResource("DiscountLimit");
...
export default class DiscountLimit extends Vue {
  public rowData: any = [];
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.posCfgInitDiscountLimit,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    formData.max_discount = parseFloat(formData.max_discount);
    this.isSaving = true;
  ...
  beforeMount() {
    ...
    this.searchOptions = [
      { text: this.$t("commons.filter.outlet"), value: 0 },
      { text: this.$t("commons.filter.userGroup"), value: 1 },
