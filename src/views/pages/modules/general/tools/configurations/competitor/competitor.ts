...
import InputForm from "@/views/pages/modules/general/tools/configurations/components/input-form/input-form.vue";
...
const resourceAPI = new ConfigurationResource("Competitor");
...
export default class Competitor extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.competitor,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    formData.total_room = parseInt(formData.total_room);
    this.isSaving = true;
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("CompetitorCategory");
      this.inputFormElement.listDropdown.category = data;
    } catch (error) {
      getError(error);
    }
  }
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      {
        headerName: this.$t("commons.table.totalRoom"),
        field: "total_room",
        width: 90,
