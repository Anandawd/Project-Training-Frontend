...
const resourceAPI = new ConfigurationResource("AccountSubGroup");
...
export default class AccountSubGroup extends Vue {
  public rowData: any = [];
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            $global.tableName.cfgInitAccountSubGroup,
            this.paramsData.id
          ),
      },
    ];
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("AccountGroup");
      this.inputFormElement.listDropdown.accountGroupName = data;
    } catch (error) {
      getError(error);
    }
  }
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      {
        headerName: this.$t("commons.table.accountGroupName"),
        field: "GroupName",
        width: 140,
      },
      {
  ...
  get schema() {
    return Yup.object().shape({
      ...
      Code: Yup.string()
        ...
        .required()
        .max(4),
      "Account Group Name": Yup.string().required(),
      "ID Sort": Yup.number().required(),
    });
  }
