...
export default class VenueCombine extends Vue {
  public rowData: any = [];
  ...
  async onChangeLocation(event: any) {
    await this.loadDropdownVenueByLocation(event.target.value);
  }
  ...
  async handleSave(formData: any) {
    this.isSaving = true;
    formData.group_number = parseInt(formData.group_number);
    formData.venue_code = formData.code;
  ...
  async editData(code: any) {
    try {
      ...
      this.loadDropdownVenueByLocation(data.location_code);
  ...
  async loadDropdownVenueByLocation(locationCode: any) {
    const paramData = {
      Param: locationCode,
    };
    try {
      const { data } = await resourceAPI.getVenueByLocation(paramData);
      this.inputFormElement.listDropdown.venueByLocation = data;
    } catch (error) {
      getError(error);
    }
  }
