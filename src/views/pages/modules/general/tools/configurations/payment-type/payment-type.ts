...
const resourceAPI = new ConfigurationResource("PaymentType");
...
export default class PaymentType extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.cfgInitPaymentType,
            this.paramsData.id
          ),
      },
    ];
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("PaymentGroup");
      this.inputFormElement.listDropdown.group = data;
    } catch (error) {
      getError(error);
    }
  }
