...
const resourceAPI = new ConfigurationResource("Store");
...
export default class Store extends Vue {
  ...
  getContextMenu(params: any) {
    ...
    const result = [
      ...
      {
        name: this.$t("commons.contextMenu.setAsDefaultStore"),
        disabled: !this.paramsData,
        action: () => this.setAsDefaultActiveStore(this.paramsData.code),
      },
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.invCfgInitStore,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    formData.is_room = parseInt(formData.is_room);
    this.isSaving = true;
  ...
  setAsDefaultActiveStore(storeCode: string) {
    this.confirmationElement.showConfirmation({
      show: true,
      params: storeCode,
      onConfirm: () => {
        this.config.setActiveStore(storeCode);
        getToastSuccess();
      },
    });
  }
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("Store");
