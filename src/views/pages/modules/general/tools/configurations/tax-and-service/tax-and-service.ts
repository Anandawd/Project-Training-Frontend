...
const resourceAPI = new ConfigurationResource("TaxAndService");
...
export default class TaxAndService extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            $global.tableName.cfgInitTaxAndService,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    this.isSaving = true;
    formData.is_tax_include = parseInt(formData.is_tax_include);
    formData.is_service_include = parseInt(formData.is_service_include);
    formData.tax = parseFloat(formData.tax);
    formData.service = parseFloat(formData.service);
    formData.service_tax = parseFloat(formData.service_tax);
  ...
  async editData(code: any) {
    try {
      ...
      this.form = data;
      this.form.is_tax_include = parseInt(this.form.is_tax_include);
      this.form.is_service_include = parseInt(this.form.is_service_include);
      this.form.tax = parseFloat(this.form.tax);
      this.form.service = parseFloat(this.form.service);
      this.form.service_tax = parseFloat(this.form.service_tax);
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      { headerName: this.$t("commons.table.name"), field: "name", width: 280 },
      {
      ...
      {
        headerName: this.$t("commons.table.serviceTax"),
        field: "service_tax",
        width: 90,
        cellStyle: { textAlign: "right" },
        headerClass: "align-header-right",
      ...
      {
        headerName: this.$t("commons.table.isTaxInclude"),
        field: "is_tax_include",
        width: 90,
      ...
      {
        headerName: this.$t("commons.table.isServiceInclude"),
        field: "is_service_include",
        width: 100,
        cellRenderer: "checklistRenderer",
        cellStyle: { textAlign: "center" },
        headerClass: "align-header-center",
