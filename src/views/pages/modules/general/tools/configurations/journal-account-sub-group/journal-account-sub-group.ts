...
const resourceAPI = new ConfigurationResource("JournalAccountSubGroup");
...
export default class JournalAccountSubGroup extends Vue {
  public rowData: any = [];
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.cfgInitJournalAccountSubGroup,
            this.paramsData.id
          ),
      },
    ];
  ...
  async loadDropdown() {
    try {
      const params = [
        "JournalAccountGroup",
        "JournalAccountSubGroupType",
        "JournalAccountSubGroup",
      ];
      const { data } = await resourceAPI.codeNameListArray(params);
      this.inputFormElement.listDropdown.journalAccountSubGroup = data;
    } catch (error) {
      getError(error);
    }
  }
  ...
  beforeMount() {
    ...
    this.searchOptions = [
      ...
      { text: this.$t("commons.filter.type"), value: 3 },
      { text: this.$t("commons.filter.parent"), value: 4 },
      { text: this.$t("commons.filter.createdBy"), value: 5 },
    ...
    this.columnDefs = [
      ...
      { headerName: this.$t("commons.table.name"), field: "name", width: 200 },
      {
      ...
      },
      {
        ...
        field: "JournalAccountSubGroupTypeName",
        width: 100,
      },
      {
        headerName: this.$t("commons.table.parent"),
        field: "Parent",
        width: 100,
      },
      {
        headerName: this.$t("commons.table.header"),
        field: "is_header",
        width: 60,
