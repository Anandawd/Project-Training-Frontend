...
const resourceAPI = new ConfigurationResource("Table");
...
export default class ProductCategory extends Vue {
  ...
  public showDialogGenerate: boolean = false;
  public options: any = {};
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.posCfgInitTable,
            this.paramsData.id
          ),
      },
    ];
  ...
  handleShowDialog(params: any, mode: any) {
    this.modeData = mode;
  ...
  async handleEdit(params: any) {
    ...
    await this.editData(params.number);
  }
  ...
  async loadDropdown() {
    try {
      const { data } = await resourceAPI.codeNameList("Outlet");
      this.inputFormElement.listDropdown.outlet = data;
    } catch (error) {
      getError(error);
    }
  }
  ...
  async generateFromRoom() {
    try {
      const { status2 } = await resourceAPI.GenerateTableRoomService();
      if (status2.status == 0) {
        this.loadData();
        this.showDialogGenerate = false;
        getToastSuccess(this.$t("messages.success"));
      }
    } catch (error) {
      getError(error);
    }
