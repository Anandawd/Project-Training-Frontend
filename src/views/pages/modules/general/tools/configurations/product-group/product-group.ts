...
const resourceAPI = new ConfigurationResource("ProductGroup");
...
export default class ProductGroup extends Vue {
      ...
      "separator",
      {
        ...
        action: () =>
          this.handleTrackingData(
            this.$global.tableName.posCfgInitProductGroup,
            this.paramsData.id
          ),
      },
    ];
  ...
  async handleSave(formData: any) {
    this.form.ItemGroupName = this.form.item_group_code;
    this.isSaving = true;
  ...
  async loadDropdown() {
    try {
      const params = ["Account", "ItemGroup"];
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      { headerName: this.$t("commons.table.name"), field: "name", width: 160 },
      {
