...
export default class ReportGrouping extends Vue {
  ...
  groupingChecker() {
    this.lengthGrouping = this.getRowData().length;
  }
  ...
  gridDrop(event: any, grid: any) {
    ...
    if (this.lengthGrouping == 3) {
      getToastError("Cannot Grouping More Than 3");
    } else {
      this.gridApi.applyTransaction({
        add: [data],
      });
