...
export default class YearlyReport extends Vue {
  ...
  gridDrop(event: any, grid: any) {
    ...
    (data.name = data.name),
      (data.code = data.code),
      (data.system_code = data.system_code),
      this.gridApi.applyTransaction({
        add: [data],
      });
  }

  insertGrid(params: any) {
    this.gridApi.applyTransaction({
      add: [params],
    });
  }
  ...
  beforeMount() {
    ...
    this.columnDefs = [
      ...
      { headerName: this.$t(""), field: "code", width: 160, hide: true },
      { headerName: this.$t(""), field: "system_code", width: 160, hide: true },
    ];
