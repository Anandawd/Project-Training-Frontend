...
@Options({
  ...
  watch: {
    modifiedRoomAvailable(val: boolean) {
      if (val) {
        this.getExpectedArrival();
      }
    },
  },
})
export default class ExpectedArrival extends Vue {
  agGridSetting: any;
  ...
  async beforeMount() {
    ...
    this.columnDefs = [
      {
        headerName: this.$t("commons.table.reservationNumber"),
        resizable: true,
        field: "number",
        width: 90,
      },
  ...
  // API CALL ================================================================================
  async getExpectedArrival() {
    try {
      const { data } = await hotelDashboardAPI.expectedArrivalDashboard();
      this.rowData = data ?? [];
