...
import WizardBedTypeAPI from "@/services/api/wizard/bed-type";
import { anyToFloat, getError } from "@/utils/general";
const wizardBedTypeAPI = new WizardBedTypeAPI();
...
export default class BedType extends Vue {
  ...
  async getList() {
    try {
      const { data } = await wizardBedTypeAPI.getList();
      this.rowData = data ?? [];
  ...
  async edit(id: number) {
    try {
      const { data } = await wizardBedTypeAPI.edit(id);
      this.form = data;
      this.showForm = true;
    } catch (error) {
      getError(error);
  ...
  async insert() {
    try {
      await wizardBedTypeAPI.insert(this.form);
      this.getList();
    } catch (error) {
      getError(error);
    }
  ...
  async update() {
    try {
      await wizardBedTypeAPI.update(this.form);
      this.getList();
    } catch (error) {
      getError(error);
    }
  ...
  async delete(code: string) {
    try {
      await wizardBedTypeAPI.delete(code);
      this.getList();
    } catch (error) {
      getError(error);
    }
  ...
  get title() {
    return (this.modeData == 1 ? "Update " : "Insert ") + " Bed Type";
  }
}
