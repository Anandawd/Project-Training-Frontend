<template>
  <div class="v-suggestions">
    <input type="text" :class="extendedOptions.inputClass" v-bind="$attrs" v-on:keydown="onKeyDown"
      v-on:blur="hideItems" v-on:focus="showItems = true" v-model="query" :autocomplete="Math.random()"
      :placeholder="extendedOptions.placeholder">
    <div class="suggestions">
      <ul class="items" v-show="items.length > 0 && showItems === true">
        <li class="item" :key="index" v-for="(item, index) in items" @click.prevent="selectItem(index)"
...
<script>
import { debounce, vue3Debounce  } from 'vue-debounce';
// import '@sass/vuexy/components/suggestions.scss';
...
export default {
  ...
  data() {
    ...
    const extendedOptions = { ...defaultOptions, ...this.options };
    return {
      extendedOptions,
      query: this.value,
      lastSetQuery: null,
